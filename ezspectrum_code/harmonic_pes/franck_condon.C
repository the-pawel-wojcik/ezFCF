...
void harmonic_FCf(arma::Mat<double> & FCf, double Mass, double dQ, 
    double Nu_ini, double Nu_targ)
{

  // Reset the FCF matrix
  FCf.fill(0);
  ...
  for (int n_ini=0; n_ini < FCf.n_rows; n_ini++ )  // for each vibr level of the initial state
  {
    for (int n_targ=0; n_targ < FCf.n_cols; n_targ++ ) // for each vibr level of the target state
    {
      ...
      for (int L=0; L <= MIN (n_ini, n_targ); L++)
      {
        for (int I=0; I <= ( ((n_targ-L)%2==0) ? (n_targ-L)/2 : (n_targ-L-1)/2 ) ; I++)
        {
          for (int J=0; J <=  ( ((n_ini-L)%2==0) ? (n_ini-L)/2 : (n_ini-L-1)/2) ; J++)
          {
            ...
            FCf(n_ini, n_targ) += tmpProduct ;
          }
        }
      }
      FCf(n_ini, n_targ) *= sqrt(Factorial(n_ini)*Factorial(n_targ)) / pow( 2, (n_ini+n_targ)/2.0 ); // * factorial fraction
                                                                                                     // multiplication by C3 (next two lines):
      FCf(n_ini, n_targ) *= exp( -d*d / (2*(a*a+1)) );
      ...
      FCf(n_ini, n_targ) *= sqrt(a*2/(a*a+1));
    }
  }
}
