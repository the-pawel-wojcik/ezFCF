...
\title{ezSpectrum v4.0}
\author{{\em Franck-Condon factors in the harmonic approximation}\\ 
\vspace{20mm}\\Vadim Mozhayskiy, Samer Gozem, and Anna I. Krylov\\ \vspace{20mm}}
\date{\today}
\maketitle
...
\section*{What is new in v.4.0}

The functionality  and most of the compute kernels are the same as in v.3, 
but the input parser was rewritten to remove the dependency on the outside library. 
The parser
  works almost as before, but with the following (temporary) restrictions:
  \begin{itemize}
...
\ezS computes stick photoelectron/photodetachment spectra for polyatomics 
...
\ezS calculations require equilibrium geometries, harmonic frequencies, 
and normal mode vectors for each electronic state. These data computed by \ai packages. 

The key feature of \ezS is a program-independent \xml input. 
...
{\tt Q-Chem}, {\tt ACESII}, {\tt Molpro}, {\tt GAMESS}, and {\tt Gaussian}
...
Please contact Professor Anna I. Krylov ({\tt krylov@usc.edu}) and Vadim Mozhayskiy 
({\tt mozhaysk@usc.edu}) if you have suggestions, questions, or bugs to report. Have fun!  

%----------------------------------------------------------------------
\section{Quick start}

In this section it is explained how to create an input \xml file from your electronic structure 
program outputs using \ul{make}{xml.py} script and how to run \ezSnospace.
...
creates an \xml input file for \ezS from several \ai outputs. 
...
Each file containstakes the in an H$_2$O frequency job using HF/6-311G**.
...
visible for \ezSnospace; for more details on comments see Sec.~\ref{sec:comments}.

To plot the spectra, use \ul{plot\_spectrum.py} script located in \ul{Plotting} directory.  

%----------------------------------------------------------------------
\subsection{Run a sample job}

\ezS executable takes the input \xml file as a single argument. 
...
To run \ezS for one of the examples type:
\begin{lstlisting}[frame=single,framerule=0pt]
  ezSpectrum samples/cis_hcoh.xml > samples/cis_hcoh.xml.out
\end{lstlisting}
...
Every \ezS output file starts with a copy of the input \xml file followed by the 
...
\ezS computes each one-dimensional harmonic FCF analytically\cite{FranckCondon:Math:30}:
...
using the normal modes of the target electronic state is prefered. 
Indeed, rotation of
...
As it was mentioned in Sec~\ref{sec:dhush:overview}, \ezS stores in memory only FCFs 
...
To overcome this problem, \ezS implements a simple automatic alignment algorithm. It aligns translational and rotational
...
which may be used for the isotope substitutions: one can rerun \ezS for a molecule with
...
\end{lstlisting}
The numbering of the normal modes starts with zero. 
\ezS prints the overlap matrix between normal modes of the initial and each target state,
...
from the \ezS output with your favorite visualization software. 
...
However, any tag which is not recognized by \ezS is simply ignored.
...
are preceded with {\tt OPT\underline{~}} and are invisible for \ezSnospace.
...
a comment and ignored by \ezSnospace.
...
An online normal modes' reordering tool may be useful (available in the online version of \ezS 
...
mkore likely, you use different normal modes in your initial and target subspaces.
...
%----------------------------------------------------------------------
\subsection{\ezS crashes with no error message!}
\label{sec:crash}

There is one known problem: \ezS crashes when too many FCFs are requested.
...
Another possibility is that you just found a new bug in \ezS :)---please let us know and forward us
...
%----------------------------------------------------------------------
\subsection{\ezS ends with a format error}
...
to show up in \ezS. Re-run the {\tt Q-Chem} job without requesting Raman to avoid this problem.
...
\addcontentsline{toc}{section}{Appendix. An overview of the \texttt{xml} format}
\label{sec:xmlformat}
...
In the case of \ezS, the input file has the {\tt <input> ... </input>} root element, 
