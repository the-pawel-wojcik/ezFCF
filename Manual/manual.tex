...
\item The closing tags, such as \begin{verbatim}</geometry>\end{verbatim} above, should be separated by either a space or a newline from the previous text. For example  \begin{verbatim}"></geometry>\end{verbatim} would result in an error.
  \end{itemize}    
The scripts were rewritten to use {\sl python3}. 
...
\ul{make}{xml.py} python script (located in {\tt InputScripts} directory)
...
\end{lstlisting}
on a Linux machine. The working directory should also contain the {\tt atomicMasses.xml} file (see Section \ref{sec:massses}. Note that the the executables must be executable---you may need to run
\begin{lstlisting}[frame=single,framerule=0pt]
  chmod +x ezFCF*.exe
\end{lstlisting}
...
A basic {\tt python} script for plotting the resulting spectra is provided in {\tt Plotting} (both {\tt python2} and {\tt python3} versions). To use it to plot a spectrum  from {\tt Samples}, follow these steps:
\begin{lstlisting}[frame=single,framerule=0pt]
  cd Samples
  ln -s ../Plotting/plot_spectrum.py .
  ./plot_spectrum.py trans_hcoh.xml.spectrum_parallel
\end{lstlisting}
The plot will be in {\tt trans\_hcoh.xml.spectrum\_parallel.pdf}. Tweak the parameters in the script for the desired appearance. (Note that {\tt python} excecutable path should be properly setup). 
