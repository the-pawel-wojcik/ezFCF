...
relaxed geometries can be extrapolated using both the gradients of the target 
state computed at the equilibrium geometry of the initial state and the initial 
state's Hessian. This approach called the vertical gradient approximation (VGA) 
is exact when both
...
VGA is triggered when the {\tt{}gradient} node is detected inside the 
{\tt{}target\_state} node. In that case, the {\tt{}geometry}, 
{\tt{}normal\_modes} and {\tt{}frequencies} nodes are ignored by the parser 
and do not need to be specified. The corresponding quantities are copied
from the \texttt{initial\_state} node just as expected in the 
parallel mode approximation without frequency shifts, after which the gradient
is used to find the VGA equilibrium geometry (as described below). The gradient must 
be given in atomic units \texttt{a.u.} (Hartree/Bohr).

If the VGA is used the \texttt{excitation\_energy} node is not allowed (it is also 
not available as the target state optimization is omitted), instead 
one has to use the analog \texttt{vertical\_excitation\_energy} node. 
...
An example of the \texttt{vertical\_excitation\_energy} and \texttt{gradient} nodes
\begin{lstlisting}[frame=single,framerule=0pt]
