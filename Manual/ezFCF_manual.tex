...
One of the challenges in the parallel approximation (Sec.~\ref{sec:parallel}) is
the requirement to optimize the target state geometry. The vertical gradient 
approximation (VGA) offers a way to extrapolate this geometry using only 
knowledge of the initial state's Hessian and the target state's gradient.

\ezFCF{} calculates a full vibronic spectrum within the parallel mode 
approximation (without frequency shifts) by using target state geometry from 
VGA. This is why the VGA are set up in the same way as parallel mode 
calculations, with the only difference in that the target state nodes contain 
the \texttt{gradient} node instead of \texttt{geometry}, \texttt{normal\_modes} 
and \texttt{frequencies}, as well as \texttt{vertical\_excitation\_energy} node 
in place of \texttt{excitation\_energy}.

An example of the \texttt{vertical\_excitation\_energy} and \texttt{gradient} 
nodes
\begin{lstlisting}[frame=single,framerule=0pt]
