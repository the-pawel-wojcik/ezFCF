...
\item Ability to compute FCFs for {\bf XXX} % TODO: Add samples and references to illustrate new features
\item Fixes to the scripts mostly extending the range of accepted {\emph ab initio} outputs.
\end{itemize}  
...
\item {\tt XXX}---an input for calculation using vertical gradient approximation {\bf Pawel, need to add sample job, and a reference file}
\item {\bf If there are new features -- need to add more examples}  
\end{itemize}
...
One can compute a full spectrum without carrying out geometry optimizations for all final states
by estimating the respective relaxed geometries using nuclear gradients computed at the equilibrium
geometry of the initial state. This can be done exactly when (i) the potential
energy surfaces are harmonic and (ii) normal modes are the same in the initial and target states.
This approach is called vertical gradient approximation and requires 
single point gradient calculations of the target states
at the minimum of the initial state potential energy surface and frequency calculation for the
initial state.


The calculations using vertical gradient approximation
are triggered by including the {\tt{}gradient} node inside the {\tt{}target\_state} node. If
the {\tt{}gradient} node is present, the {\tt{}geometry} node is taken as the single point calculations
geometry (the initial state geometry). The {\tt{}normal\_modes} and {\tt{}frequencies} nodes must also correspond to the initial state (usually, the ground state), just as expected in the parallel mode
approximation without frequency shifts. The meaning of the {\tt{}excitation\_energy} node changes
as well---it now represents the vertical excitation energy. In the current implementation,
...
of  the potential energy surface of the target state in Cartesian coordinates evaluated at the geometry of the ground state. The other symbols are explained in Sec.~\ref{sec:dhush:overview}. The adiabatic excitation energy (bottom to bottom) is given by
\begin{equation}
    \label{eq:VG_energy}
